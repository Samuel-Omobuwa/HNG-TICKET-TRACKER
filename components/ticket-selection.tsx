...
interface TicketSelectionProps {
  onNext: (data: { type: string; quantity: number }) => void
}

export default function TicketSelection({ onNext }: TicketSelectionProps) {
  const [selectedTicket, setSelectedTicket] = useState<string | null>(null)
  const [ticketCount, setTicketCount] = useState(1)

  const handleNext = () => {
    if (selectedTicket) {
      onNext({ type: selectedTicket, quantity: ticketCount })
    }
  }
